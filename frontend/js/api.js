...
const API = {
  ...
  // Log the download server-side (increments counter) then trigger real file download
  async download(id) {
    const token = getToken();
    // 1. Tell server to log it and get file info
    const data = await request('POST', `/software/${id}/download`);
    // 2. If server has a real file, trigger browser download via direct URL
    if (data.hasFile) {
      const a = document.createElement('a');
      a.href = `/api/software/${id}/file?token=${encodeURIComponent(token)}`;
      a.download = data.filename || data.software.name;
